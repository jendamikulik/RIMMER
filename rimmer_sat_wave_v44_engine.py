...
"""
rimmer_sat_wave_v44_engine.py

V4.4 engine:
- exact / projected dual mode
- DIMACS loading
- Trotter evolution
- SAT-targeted reweighting pulse
- adaptive mixing
- guided resampling in projected mode
- collapse / ABS / monodromy
- SAT vs frustration diagnostics
...
Controls:
  ...
  s     : resample projected subspace
  g     : guided resample projected subspace
  w     : SAT-targeted reweight pulse
"""
...
class ExactSATWaveEngine:
    ...
    def reweight_pulse(self, alpha=0.6):
        self.psi *= np.exp(-alpha * self.H_clause_diag)
        self.normalize()
...
class ProjectedSATWaveEngine:
    ...
    def guided_candidates(self, seeds):
        out = []
        for bits in seeds:
            out.append(bits)
            for b in range(self.n):
    ...
    def resample(self):
        ...
        self.guided_resample(initial=True)

    def guided_resample(self, initial=False):
        if initial or self.psi is None:
            seeds = self.sample_bits[: min(32, len(self.sample_bits))]
        else:
            p = self.probs()
            idx = np.argsort(-p)[: min(32, len(p))]
            seeds = [self.sample_bits[i] for i in idx]

        candidates = set(self.guided_candidates(seeds))
        while len(candidates) < self.k_samples:
            candidates.add(self.random_bits())

        # choose best by violations, then diversify with random remainder
        cand_list = list(candidates)
        cand_list.sort(key=lambda b: (violations(b, self.clauses), self.rng.random()))
        selected = []
        seen = set()
        for b in cand_list:
            if b not in seen:
                selected.append(b)
                seen.add(b)
            if len(selected) >= self.k_samples:
                break

        self.sample_bits = selected[: self.k_samples]
        self._rebuild_subspace()
        K = len(self.sample_bits)
        self.psi = np.ones(K, dtype=complex) / np.sqrt(K)
        self.last_event = "guided-resample" if not initial else "resample"
        self.last_index = None
        self.monodromy_count = 0
        self.t = 0.0
...
class RimmerSATWaveApp:
    def __init__(self):
        ...
        self.ax_info = self.fig.add_subplot(gs[1, 2]); self.ax_info.axis("off")

        self.fig.canvas.manager.set_window_title("Rimmer SAT Wave V4.4 — engine")
    ...
    def reset_history(self):
        self.t_hist, self.sat_hist, self.frus_hist, self.energy_hist = [], [], [], []
    ...
    def refresh(self):
        ...
        x = np.arange(len(idx_order)); w = 0.38
        ...
        self.ax_compare.set_title(f"preview at measured index {self.engine.labels[meas_idx]}")
        ...
        info = (
            ...
            f"[m]=monodromy [w]=reweight [i]=builtin\n"
            f"[l]=load DIMACS [r]=reset [s]=resample [g]=guided"
        )
        self.ax_info.clear()
    ...
    def on_key(self, event):
        ...
        try:
            if key == " ":
                ...
                self.reset_history()
            elif key == "g" and hasattr(self.engine, "guided_resample"):
                self.engine.guided_resample()
                self.reset_history()
